...
class PokerHandEvaluator:
    ...
    def generate_one_card_low_suit_hands(self, suit='s'):
        ...
        for combo in one_card_combos:
            ...
            # Find the lowest card of the specified suit in the hand
            suit_cards = [card for card in hand if card[-1] == suit]
            if suit_cards:
                # Get the rank of the lowest card of the specified suit
                lowest_suit_card = min(suit_cards, key=lambda x: ranks.index(x[:-1]))
                rank_idx = ranks.index(lowest_suit_card[:-1]) + 1  # 1 for Ace, 2 for King, etc.
            else:
...
for suit in suits:
    ...
    all_hands = hand_evaluator.generate_one_card_low_suit_hands(suit=suit)
    ...
    if not_in_all_hands:
        print(f"The following hands were not generated for low {suit_names[suit]} hands:")
        print(not_in_all_hands)
    else:
        print(f"All hands accounted for in low {suit_names[suit]} ranking.")

    # Write to CSV file
    output_file = f'all_card_hands_ranked_one_card_low_{suit_names[suit]}.csv'
