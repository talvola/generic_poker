...
class PokerTable {
    ...
    connectToTable() {
        this.socket.emit('join_table', { table_id: this.tableId });
    }
    ...
    renderPlayers() {
        ...
        Object.values(this.players).forEach((player, index) => {
            const seat = this.createPlayerSeat(player, index);
    ...
    createPlayerSeat(player, position) {
        ...
        seat.innerHTML = `
            <div class=\"${playerInfoClass}\">
                ...
                <div class=\"player-chips\">$${player.chip_stack}</div>
    ...
    createActionButton(action) {
        ...
        let buttonText = action.type.charAt(0).toUpperCase() + action.type.slice(1);
