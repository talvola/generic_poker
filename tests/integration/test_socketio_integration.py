...
def _setup_two_player_game(app, socketio, player1, player2, holdem_table):
    ...
    http2.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": 100, "seat_number": 2})
...
class TestReadyAndHandStart:
    ...
    def test_both_players_ready_starts_hand(self, app, socketio, player1, player2, holdem_table):
        ...
        sio1, sio2, table_id = _setup_two_player_game(app, socketio, player1, player2, holdem_table)
...
class TestPlayerActions:
    ...
    def test_fold_ends_hand(self, app, socketio, player1, player2, holdem_table):
        ...
        cp_sio.emit("player_action", {"table_id": table_id, "action": "fold", "amount": 0})
    ...
    def test_call_and_check_via_socketio(self, app, socketio, player1, player2, holdem_table):
        ...
        for _ in range(2):
            ...
            cp_sio.get_received()
            if "check" in names:
                cp_sio.emit("player_action", {"table_id": table_id, "action": "check", "amount": 0})
            elif "call" in names:
                call_info = _find_action(actions, "call")
                cp_sio.emit("player_action", {"table_id": table_id, "action": "call", "amount": call_info[1]})
    ...
    def test_full_hand_to_showdown(self, app, socketio, player1, player2, holdem_table):
        ...
        while game.state not in [GameState.COMPLETE, GameState.SHOWDOWN] and action_count < max_actions:
            ...
            action_results = [e for e in events if e["name"] == "action_result"]
            if action_results:
                assert action_results[0]["args"][0]["success"] is True, f"Action failed: {action_results[0]['args'][0]}"
...
class TestGameStateUpdates:
    ...
    def test_opponent_receives_action_broadcast(self, app, socketio, player1, player2, holdem_table):
        ...
        assert "player_action" in other_names, f"Other player didn't get player_action, got: {other_names}"
        assert "game_state_update" in other_names, f"Other player didn't get game_state_update, got: {other_names}"
    ...
    def test_game_state_hides_opponent_cards(self, app, socketio, player1, player2, holdem_table):
        ...
        for p in state.get("players", []):
            if p["user_id"] == player1.id:
                ...
                assert len(p.get("cards", [])) == 0, f"Opponent cards should be hidden, got: {p.get('cards')}"
    ...
    def test_blinds_reflected_in_game_state(self, app, socketio, player1, player2, holdem_table):
        ...
        assert pot.get("total_pot", 0) == 3, f"Expected pot=3 (1+2 blinds), got {pot.get('total_pot', 0)}"
        ...
        assert total_stacks == 197, f"Expected total stacks=197 (200-3 blinds), got {total_stacks}: {stacks}"

        # At least one player should have a current_bet > 0
        bets = {p["user_id"]: p.get("current_bet", 0) for p in state.get("players", [])}
        assert any(b > 0 for b in bets.values()), f"Expected at least one player with current_bet > 0, got {bets}"
    ...
    def test_phase_detection_preflop(self, app, socketio, player1, player2, holdem_table):
        ...
        assert state is not None
        assert state.get("game_phase") == "preflop", f"Expected phase=preflop, got {state.get('game_phase')}"
    ...
    def test_phase_transitions_through_streets(self, app, socketio, player1, player2, holdem_table):
        ...
        assert "preflop" in phases_seen, f"Expected preflop phase, phases seen: {phases_seen}"
        # If game reached flop, check it was detected
        if action_count > 2:
            assert len(phases_seen) > 1, f"Expected phase transitions, only saw: {phases_seen}"
    ...
    def test_hand_complete_broadcasts_results(self, app, socketio, player1, player2, holdem_table):
        ...
        assert results.get("total_pot", 0) > 0, f"Expected total_pot > 0, got {results}"
        assert len(results.get("pots", [])) > 0, f"Expected at least one pot result, got {results}"
...
class TestFullHandCycle:
    ...
    def test_full_hand_then_second_hand(self, app, socketio, player1, player2, holdem_table):
        ...
        assert game.state in [GameState.COMPLETE, GameState.SHOWDOWN], f"Hand 1 didn't complete, state: {game.state}"
        ...
        assert total_after_h1 == 200, f"Expected 200 total chips after hand 1, got {total_after_h1}: {stacks_after_h1}"
        ...
        assert game.state in [GameState.BETTING, GameState.DEALING], f"Hand 2 didn't start, state: {game.state}"
        assert game.current_player is not None, "No current player in hand 2"

        # Play hand 2
        _play_hand_to_completion(game, sio1, sio2, player1.id, table_id)

        assert game.state in [GameState.COMPLETE, GameState.SHOWDOWN], f"Hand 2 didn't complete, state: {game.state}"
        ...
        assert total_after_h2 == 200, f"Expected 200 total chips after hand 2, got {total_after_h2}: {stacks_after_h2}"
    ...
    def test_winner_gets_pot_and_stacks_update(self, app, socketio, player1, player2, holdem_table):
        ...
        assert stacks_after[other_id] > 100, f"Expected winner to have >100 chips, got {stacks_after[other_id]}"
...
class TestLeaveAndRejoin:
    ...
    def test_leave_removes_player_from_game_session(self, app, socketio, player1, player2, holdem_table):
        ...
        assert (
            player1.id not in session.connected_players
        ), "Player 1 should be removed from game session after leaving mid-hand"
        assert player2.id in session.connected_players, "Player 2 should still be in game session"
    ...
    def test_rejoin_after_leaving(self, app, socketio, player1, player2, holdem_table):
        ...
        resp = http1_new.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": 100, "seat_number": 1})
    ...
    def test_leave_mid_hand_awards_pot_to_remaining_player(self, app, socketio, player1, player2, holdem_table):
        ...
        assert (
            p2_stack_after > p2_stack_before
        ), f"Winner should have more chips than {p2_stack_before}, got {p2_stack_after}"

        # Player 1 should be removed from the game
        assert player1.id not in session.connected_players, "Leaving player should be removed from session"
    ...
    def test_leave_mid_hand_preserves_disconnect_behavior(self, app, socketio, player1, player2, holdem_table):
        ...
        is_current = session.game.current_player and session.game.current_player.id == player1.id

        dm.handle_player_disconnect(player1.id, table_id, is_current)

        # Player should be tracked as disconnected
        assert dm.is_player_disconnected(player1.id), "Disconnected player should be tracked by disconnect manager"

        # Player should NOT be in pending_leaves (that's for intentional leave)
        assert player1.id not in session.pending_leaves, "Disconnected player should NOT be in pending_leaves"
...
def _setup_two_player_game_custom(app, socketio, player1, player2, holdem_table, buy_in1=100, buy_in2=100):
    ...
    http1, sio1 = _login_and_connect(app, socketio, player1)
    http1.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": buy_in1, "seat_number": 1})

    http2, sio2 = _login_and_connect(app, socketio, player2)
    http2.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": buy_in2, "seat_number": 2})
...
class TestAllInViaSocketIO:
    ...
    def test_all_in_preflop_completes_hand(self, app, socketio, player1, player2, holdem_table):
        ...
        _, sio1, _, sio2, table_id = _setup_two_player_game_custom(
            app, socketio, player1, player2, holdem_table, buy_in1=40, buy_in2=200
        )
        ...
        cp_sio.emit(
            "player_action",
            {
                "table_id": table_id,
                "action": "raise",
                "amount": cp_stack + game.betting.current_bets.get(cp.id, type("", (), {"amount": 0})).amount,
            },
        )
        all_events = list(sio1.get_received()) + list(sio2.get_received())

        # Other player calls
        other_sio.emit("player_action", {"table_id": table_id, "action": "call", "amount": game.betting.current_bet})
        all_events.extend(sio1.get_received())
        all_events.extend(sio2.get_received())

        # Play through remaining streets (game should handle all-in progression)
        remaining_events = _collect_events_through_hand(game, sio1, sio2, player1.id, table_id)
        ...
        assert len(hc_events) >= 1, f"No hand_complete event. Events: {set(e['name'] for e in all_events)}"
        ...
        assert p1_stack + p2_stack == 240, f"Expected 240 total chips, got {p1_stack + p2_stack}"
    ...
    def test_all_in_shows_zero_stack_in_game_state(self, app, socketio, player1, player2, holdem_table):
        ...
        # Go all-in
        cp_total = (
            game.table.players[cp_id].stack + game.betting.current_bets.get(cp_id, type("", (), {"amount": 0})).amount
        )
        sio1.get_received()
        sio2.get_received()
        cp_sio.emit("player_action", {"table_id": table_id, "action": "raise", "amount": cp_total})
        sio1.get_received()
        sio2.get_received()
        ...
        assert (
            player_states[cp_id]["chip_stack"] == 0
    ...
    def test_both_all_in_game_completes(self, app, socketio, player1, player2, holdem_table):
        ...
        cp_sio.emit("player_action", {"table_id": table_id, "action": "raise", "amount": 100})
        sio1.get_received()
        sio2.get_received()

        # Other player calls all-in
        other_sio.emit("player_action", {"table_id": table_id, "action": "call", "amount": 100})
        all_events = list(sio1.get_received()) + list(sio2.get_received())

        # Both are all-in, hand should auto-complete through remaining streets
        remaining = _collect_events_through_hand(game, sio1, sio2, player1.id, table_id)
        all_events.extend(remaining)

        assert game.state in [GameState.COMPLETE, GameState.SHOWDOWN], f"Expected complete, got {game.state}"

        hc_events = [e for e in all_events if e["name"] == "hand_complete"]
        assert len(hc_events) >= 1, "Should have hand_complete event"

        results = hc_events[0]["args"][0].get("hand_results", {})
        assert results.get("total_pot") == 200, f"Expected pot of 200, got {results.get('total_pot')}"
...
class TestSplitPotViaSocketIO:
    ...
    def test_split_pot_hand_complete_event(self, app, socketio, player1, player2, holdem_table):
        ...
        all_events = _collect_events_through_hand(game, sio1, sio2, player1.id, table_id)
        ...
        assert len(winners) == 2, f"Expected 2 winners (split), got {len(winners)}: {winners}"
...
class TestRaiseViaSocketIO:
    ...
    def test_raise_and_call_via_socketio(self, app, socketio, player1, player2, holdem_table):
        ...
        cp_sio.emit("player_action", {"table_id": table_id, "action": "raise", "amount": 10})
        sio1.get_received()
        sio2.get_received()

        # Other player calls
        other_sio.emit("player_action", {"table_id": table_id, "action": "call", "amount": 10})
        sio1.get_received()
        sio2.get_received()
    ...
    def test_raise_reraise_call_via_socketio(self, app, socketio, player1, player2, holdem_table):
        ...
        # p1 raises to 6
        cp_sio.emit("player_action", {"table_id": table_id, "action": "raise", "amount": 6})
        sio1.get_received()
        sio2.get_received()

        # p2 3-bets to 18
        other_sio.emit("player_action", {"table_id": table_id, "action": "raise", "amount": 18})
        sio1.get_received()
        sio2.get_received()

        # p1 calls the 3-bet
        cp_sio.emit("player_action", {"table_id": table_id, "action": "call", "amount": 18})
        sio1.get_received()
        sio2.get_received()
    ...
    def test_post_flop_bet_raise_via_socketio(self, app, socketio, player1, player2, holdem_table):
        ...
        # SB calls
        cp_sio.emit("player_action", {"table_id": table_id, "action": "call", "amount": 2})
        sio1.get_received()
        sio2.get_received()

        # BB checks
        other_sio.emit("player_action", {"table_id": table_id, "action": "check", "amount": 0})
        sio1.get_received()
        sio2.get_received()
        ...
        flop_cp_sio.emit("player_action", {"table_id": table_id, "action": "bet", "amount": 10})
        sio1.get_received()
        sio2.get_received()

        # Other player raises to 30
        flop_other_sio.emit("player_action", {"table_id": table_id, "action": "raise", "amount": 30})
        sio1.get_received()
        sio2.get_received()

        # First player calls
        flop_cp_sio.emit("player_action", {"table_id": table_id, "action": "call", "amount": 30})
        sio1.get_received()
        sio2.get_received()
...
class TestDrawGameplay:
    ...
    def test_draw_action_via_http_api(self, app, socketio, player1, player2, draw_table):
        ...
        http_client = app.test_client()
        http_client.post("/auth/api/login", json={"username": test_username, "password": "password123"})

        # Send draw action via HTTP API
        resp = http_client.post(f"/game/sessions/{table_id}/action", json={"action": "draw", "cards": cards_to_discard})
...
def _setup_pass_game(app, socketio, player1, player2, player3, pass_table):
    ...
    http1, sio1 = _login_and_connect(app, socketio, player1)
    http1.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": 200, "seat_number": 1})

    http2, sio2 = _login_and_connect(app, socketio, player2)
    http2.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": 200, "seat_number": 2})

    http3, sio3 = _login_and_connect(app, socketio, player3)
    http3.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": 200, "seat_number": 3})
...
class TestPassGameplay:
    ...
    def test_pass_phase_appears_in_state(self, app, socketio, player1, player2, player3, pass_table):
        ...
        sio1, sio2, sio3, table_id = _setup_pass_game(app, socketio, player1, player2, player3, pass_table)
...
# ── Stud 8 (Hi-Lo) Game Tests ───────────────────────────────────────────────


@pytest.fixture
def stud8_table(app, player1):
    """Create a 5-Card Stud 8 (Hi-Lo) table with bring-in."""
    table_manager = TableManager()
    config = TableConfig(
        name="Test 5-Card Stud 8",
        variant="5_card_stud_8",
        betting_structure=BettingStructure.LIMIT,
...
def _setup_stud8_game(app, socketio, player1, player2, stud8_table):
    """Join both players, connect, set ready for stud 8 game.

    Returns (sio1, sio2, table_id).
    """
    table_id = str(stud8_table.id)

    http1, sio1 = _login_and_connect(app, socketio, player1)
    http1.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": 400, "seat_number": 1})

    http2, sio2 = _login_and_connect(app, socketio, player2)
    http2.post(f"/api/tables/{table_id}/join", json={"buy_in_amount": 400, "seat_number": 2})
...
def _play_stud_hand_passively(game, sio1, sio2, player1_id, table_id, max_actions=80):
    """Play a stud hand to completion using passive actions (bring_in/check/call).

    Handles bring-in, complete, and all standard betting actions.
    Returns all collected SocketIO events.
    """
    all_events = []
    action_count = 0
    ...
    while game.state not in [GameState.COMPLETE, GameState.SHOWDOWN] and action_count < max_actions:
        ...
        # Choose passive action: bring_in > check > call > fold
        if "bring_in" in names:
            bi_action = _find_action(actions, "bring_in")
            cp_sio.emit(
                "player_action",
                {"table_id": table_id, "action": "bring_in", "amount": bi_action[1] if len(bi_action) > 1 else 0},
            )
        elif "check" in names:
            cp_sio.emit("player_action", {"table_id": table_id, "action": "check", "amount": 0})
        elif "call" in names:
...
class TestStud8Gameplay:
    """Test 5-Card Stud 8 (Hi-Lo with bring-in) through the online platform."""

    def test_stud8_hand_starts(self, app, socketio, player1, player2, stud8_table):
        """5-Card Stud 8 game starts with antes posted and bring-in required."""
        sio1, sio2, table_id = _setup_stud8_game(app, socketio, player1, player2, stud8_table)
        ...
        assert game is not None
        assert game.state == GameState.BETTING, f"Expected BETTING, got {game.state}"
        assert game.current_player is not None, "Should have a current player"

        # Current player should have bring_in as an action
        actions = game.get_valid_actions(game.current_player.id)
        names = _action_names(actions)
        assert "bring_in" in names, f"Expected bring_in action, got {names}"

        sio1.disconnect()
        sio2.disconnect()

    def test_stud8_full_hand_to_showdown(self, app, socketio, player1, player2, stud8_table):
        """Play a complete 5-Card Stud 8 hand through showdown via SocketIO."""
        sio1, sio2, table_id = _setup_stud8_game(app, socketio, player1, player2, stud8_table)
        ...
        # Play hand passively
        all_events = _play_stud_hand_passively(game, sio1, sio2, player1.id, table_id)

        assert game.state in [GameState.COMPLETE, GameState.SHOWDOWN], f"Game didn't complete, state: {game.state}"

        # Verify hand results exist
        hand_results = game.get_hand_results()
        assert hand_results is not None
        assert len(hand_results.pots) > 0
        assert hand_results.total_pot > 0

        # Verify hand_complete event was broadcast
        hc_events = [e for e in all_events if e["name"] == "hand_complete"]
        assert len(hc_events) >= 1, f"No hand_complete event. Events: {set(e['name'] for e in all_events)}"

        # Verify the results dict in hand_complete is valid JSON-serializable
        hc_data = hc_events[0]["args"][0]
        results = hc_data.get("hand_results", {})
        assert results.get("total_pot", 0) > 0, f"Expected total_pot > 0, got {results}"
        assert len(results.get("pots", [])) > 0, f"Expected at least one pot result, got {results}"

        # For hi-lo game, verify hands dict has entries
        assert len(results.get("hands", {})) > 0, f"Expected hands in results, got {results}"

        # Verify winning_hands exist
        assert len(results.get("winning_hands", [])) > 0, f"Expected winning_hands in results, got {results}"

        # Verify player hole cards are included (for showdown display)
        assert len(results.get("player_hole_cards", {})) > 0, f"Expected player_hole_cards in results, got {results}"

        sio1.disconnect()
        sio2.disconnect()

    def test_stud8_chip_conservation(self, app, socketio, player1, player2, stud8_table):
        """Chips are conserved through a complete 5-Card Stud 8 hand."""
        sio1, sio2, table_id = _setup_stud8_game(app, socketio, player1, player2, stud8_table)

        session = game_orchestrator.get_session(table_id)
        game = session.game

        # Total buy-in: 400 * 2 = 800
        total_buyin = 800

        sio1.get_received()
        sio2.get_received()

        _play_stud_hand_passively(game, sio1, sio2, player1.id, table_id)
        ...
        p2_stack = game.table.players[player2.id].stack
        assert (
            p1_stack + p2_stack == total_buyin
        ), f"Expected {total_buyin} total chips, got {p1_stack + p2_stack} ({p1_stack} + {p2_stack})"
