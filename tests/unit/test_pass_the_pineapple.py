...
def setup_test_game_with_mock_deck():
    ...
    rules = load_rules_from_file('pass_the_pineapple')
...
def test_game_results_showdown_high():
    ...
    # only action is PASS of 1 card
    assert len(valid_actions) == 1
    assert any(action[0] == PlayerAction.PASS and action[1] == 1 and action[2] == 1 for action in valid_actions)    

    passing_player = game.current_player.id
    cards_to_pass = game.table.players[passing_player].hand.cards[:1]  # pass the first card
    game.player_action(passing_player, PlayerAction.PASS, cards=cards_to_pass)
    ...
    # Verify hole cards
    assert str(game.table.players['BTN'].hand.cards[0]) == "Kh"  # Alice
    assert str(game.table.players['BTN'].hand.cards[1]) == "Qh"  # Alice
    assert str(game.table.players['BTN'].hand.cards[2]) == "Js"  # Alice received from Charlie
    assert str(game.table.players['SB'].hand.cards[0]) == "Kc"  # Bob
    assert str(game.table.players['SB'].hand.cards[1]) == "Kd"  # Bob
    assert str(game.table.players['SB'].hand.cards[2]) == "Ah"  # Bob recewived from Alice
    assert str(game.table.players['BB'].hand.cards[0]) == "Jd"  # Charlie
    assert str(game.table.players['BB'].hand.cards[1]) == "Ts"  # Charlie
    assert str(game.table.players['BB'].hand.cards[2]) == "Qd"  # Charlie received from Bob
    ...
    assert main_pot.split  # split pot!
    ...
    assert "Queens over Jacks" in winning_hand[0].hand_description
