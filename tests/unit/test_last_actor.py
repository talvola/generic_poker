...
def create_test_game(mock_hands=None):
    ...
    rules = {
        ...
        "gamePlay": [
            ...
            {"deal": {"location": "player", "cards": [{"number": 1, "state": "face down"}]}, "name": "Deal Hole Cards"},
...
def test_last_actor_all_in_scenario():
    ...
    game.player_action("p4", PlayerAction.CALL, 10)
    game.player_action(
        "p1", PlayerAction.RAISE, 15
    )  # All-in for less than normal raise amount of 20 - should be allowed
    ...
    # p4 was last to act, so p1 is next in order — but p1 is all-in (stack 0)
    # and cannot act on later streets, so action starts with p2
    game._next_step()  # Deal
    game._next_step()  # Next betting round
...
def test_last_actor_cap_betting_limit():
    ...
    game.player_action("p4", PlayerAction.CALL, 40)
