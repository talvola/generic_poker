...
class TestSessionManager:
    ...
    def test_get_current_user_info_authenticated(self, app_context, test_user):
        ...
        with (
            patch("src.online_poker.services.auth_service.current_user", mock_current_user),
            patch("src.online_poker.services.auth_service.session", mock_session),
            patch.object(SessionManager, "_is_session_valid", return_value=True),
        ):
    ...
    def test_refresh_session_success(self, mock_current_user, app_context, test_user):
        ...
        with patch("flask.session", {}), patch("src.online_poker.database.db.session.commit"):
    ...
    def test_is_session_valid_expired_session(self, app_context):
        ...
        with patch(
            "flask.session",
            {"user_id": "test-id", "session_token": "test-token", "login_time": old_time, "permanent": False},
        ):
            result = SessionManager._is_session_valid()
    ...
    def test_is_session_valid_inactive_user(self, app_context, test_user):
        ...
        with patch(
            "flask.session",
            {"user_id": test_user.id, "session_token": "test-token", "login_time": datetime.utcnow().isoformat()},
        ):
            result = SessionManager._is_session_valid()
...
class TestPasswordResetService:
    ...
    def test_reset_password_expired_token(self, app_context, test_user):
        ...
        # Mock session methods (iteration, key access, pop)
        mock_session.__iter__ = lambda self: iter(session_data)
        ...
        mock_session.startswith = session_data.__contains__
    ...
    def test_cleanup_expired_tokens(self, app_context):
        ...
        session["reset_token_valid"] = {"token_hash": "valid-hash", "expires_at": valid_time, "user_id": "valid-user"}
