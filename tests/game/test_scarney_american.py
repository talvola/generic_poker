...
def create_predetermined_deck():
    ...
    named_cards = [
        ...
        Card(Rank.SIX, Suit.CLUBS), Card(Rank.KING, Suit.DIAMONDS), Card(Rank.FOUR, Suit.CLUBS),
        Card(Rank.SEVEN, Suit.HEARTS), Card(Rank.QUEEN, Suit.SPADES), Card(Rank.JACK, Suit.HEARTS),
        Card(Rank.EIGHT, Suit.SPADES), Card(Rank.TEN, Suit.HEARTS), Card(Rank.ACE, Suit.CLUBS),
        Card(Rank.NINE, Suit.HEARTS), Card(Rank.TWO, Suit.DIAMONDS), Card(Rank.FOUR, Suit.HEARTS),
        # flop
...
def test_scarney_minimal_flow():
    ...
    cards_to_discard = hand.get_cards()[:1]
    game.player_action(pid, PlayerAction.DISCARD, cards=cards_to_discard)
    assert len(hand.get_cards()) == initial_count - 5  # Hand size reduced by discards     
    pid = 'BTN'
    hand = game.table.players[pid].hand
    initial_count = len(hand.get_cards())
    cards_to_discard = hand.get_cards()[:1]
    ...
    assert len(game.table.community_cards['Discard']) == 5 + 7  # 6 discards added to Discard board
    ...
    assert len(game.table.community_cards['Discard']) == 6 + 7 + 3 # 3 discards added to Discard board
    ...
    # American/Canadian test - in American we bet if we have zero hole cards
    ...
    assert winning_hand[0].hand_description == 'Full House, Kings over Sevens'  
