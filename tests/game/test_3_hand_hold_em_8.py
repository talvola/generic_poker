"""Minimal end-to-end test for 3_hand_hold_em_8."""
import pytest
...
def setup_test_game():
    """Setup a 3-player 3_hand_hold_em_8 game with a mock deck."""
    rules = load_rules_from_file('3_hand_hold_em_8')
    game = Game(
        rules=rules,
...
def test_3_hand_hold_em_8_minimal_flow():
    """Test minimal flow for 3_hand_hold_em_8 from start to showdown."""
    ...
    game._next_step()  # separate_cards
    ...
    assert any(a[0] == PlayerAction.SEPARATE and a[1] == 6 for a in actions)
    for pid in ['SB', 'BB', 'BTN']:
        hand = game.table.players[pid].hand
        cards = hand.get_cards()[:6]
        game.player_action(pid, PlayerAction.SEPARATE, cards=cards)
        assert len(hand.get_subset('Hand 1')) == 2  # Hand 1
        assert len(hand.get_subset('Hand 2')) == 2  # Hand 2
        assert len(hand.get_subset('Hand 3')) == 2  # Hand 3
    print(f'\nStep 2 - Post-Separate Hands:')
    for pid in ['SB', 'BB', 'BTN']:
        print(f'{pid} - Hand 1: {[str(c) for c in game.table.players[pid].hand.get_subset("Hand 1")]}')
        print(f'{pid} - Hand 2: {[str(c) for c in game.table.players[pid].hand.get_subset("Hand 2")]}')
        print(f'{pid} - Hand 3: {[str(c) for c in game.table.players[pid].hand.get_subset("Hand 3")]}')
    # Step 3: Pre-Flop Bet
    ...
    # Step 5: Discard One Hand
    game._next_step()  # discard_one_hand
    ...
    assert winning_hand[0].hand_description == 'Ace-high Flush'  # Updated from run
