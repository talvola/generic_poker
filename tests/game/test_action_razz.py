"""Tests for action razz end-to-end."""
...
def setup_test_game_with_mock_deck():
    ...
    rules = load_rules_from_file("action_razz")
...
def test_game_bringin():
    ...
    # test current_bets in BettingManager
    assert game.betting.current_bets == {"p2": PlayerBet(amount=3, has_acted=False, posted_blind=True, is_all_in=False)}
    # also test total_bets and total_antes from Pot
    ...
    # complete to $10
    result = game.player_action("p3", PlayerAction.COMPLETE, 10)
    assert result.success
    ...
    # hands:
    # Alice: Ah,Kh,Qh,Ts,Jh,9s,9d
    # Bob: Qd,Kc,Kd,Qc,Td,7s,7h
    # Charlie: Js,Jd,2c,Qs,Th,6s,6c
...
def create_predetermined_deck_single_face():
    """Create a deck with predetermined cards for testing."""
    # Create cards in the desired deal order (first card will be dealt first)
    # In this three-handed case, button is dealt first, then SB, then BB in order
    cards = [
        Card(Rank.KING, Suit.HEARTS),  # Alice FD HOLE
        Card(Rank.SEVEN, Suit.DIAMONDS),  # Bob FD HOLE
        Card(Rank.SIX, Suit.SPADES),  # Charlie FD HOLE
        Card(Rank.FIVE, Suit.HEARTS),  # Alice FD HOLE2
        Card(Rank.FOUR, Suit.CLUBS),  # Bob FD HOLE2
        Card(Rank.THREE, Suit.DIAMONDS),  # Charlie FD HOLE2
        Card(Rank.TWO, Suit.HEARTS),  # Alice DOOR CARD
        Card(Rank.EIGHT, Suit.DIAMONDS),  # Bob DOOR CARD
        Card(Rank.TWO, Suit.CLUBS),  # Charlie DOOR CARD
        Card(Rank.JACK, Suit.SPADES),  # Alice 4TH ST
        Card(Rank.NINE, Suit.CLUBS),  # Bob 4TH ST
        Card(Rank.NINE, Suit.SPADES),  # Charlie 4TH ST
        Card(Rank.EIGHT, Suit.HEARTS),  # Alice 5TH ST
        Card(Rank.TEN, Suit.DIAMONDS),  # Bob 5TH ST
        Card(Rank.TEN, Suit.HEARTS),  # Charlie 5TH ST
        Card(Rank.NINE, Suit.SPADES),  # Alice 6TH ST
        Card(Rank.SEVEN, Suit.SPADES),  # Bob 6TH ST
        Card(Rank.SIX, Suit.HEARTS),  # Charlie 6TH ST
        Card(Rank.NINE, Suit.DIAMONDS),  # Alice 7TH ST
        Card(Rank.SEVEN, Suit.HEARTS),  # Bob 7TH ST
        Card(Rank.SIX, Suit.CLUBS),  # Charlie 7TH ST
        # Rest of the deck in some order (won't be used in 5-card poker)
    ...
    # hands:
    # Alice: Kh,5h,2h,Js,8h,9s,9d
    # Bob: 7d,4c,8d,9c,Td,7s,7h
    # Charlie: 6s,3d,2c,9s,Th,6h,6c


# Alice has Face hand.   Bob and Charlie have Butt hands.
# Alice has T,9,8,5,2
# Bob has T,9,8,7,4
# Charlie has T,9,6,3,2


def setup_test_game_with_mock_deck_specified(deck):
    """Create a test game with three players and a predetermined deck."""
    rules = load_rules_from_file("action_razz")
...
def test_game_single_face():
    """Test that the game results API provides correct information."""
    game = setup_test_game_with_mock_deck_specified(create_predetermined_deck_single_face())
    ...
    assert "High Card" in winning_hand[0].hand_name
    assert "Jack High" in winning_hand[0].hand_description
...
def create_predetermined_deck_two_face():
    """Create a deck with predetermined cards for testing."""
    # Create cards in the desired deal order (first card will be dealt first)
    # In this three-handed case, button is dealt first, then SB, then BB in order
    cards = [
        Card(Rank.KING, Suit.HEARTS),  # Alice FD HOLE
        Card(Rank.KING, Suit.DIAMONDS),  # Bob FD HOLE
        Card(Rank.SIX, Suit.SPADES),  # Charlie FD HOLE
        Card(Rank.FIVE, Suit.HEARTS),  # Alice FD HOLE2
        Card(Rank.FOUR, Suit.CLUBS),  # Bob FD HOLE2
        Card(Rank.THREE, Suit.DIAMONDS),  # Charlie FD HOLE2
    ...
    # hands:
    # Alice: Kh,5h,2h,Js,8h,9s,9d
    # Bob: Kd,4c,8d,9c,Td,7s,7h
    # Charlie: 6s,3d,2c,9s,Th,6h,6c


# Alice and Bob have Face hand.   Charlie have Butt hands.
# Alice has J,9,8,5,2
# Bob has T,9,8,7,4
# Charlie has T,9,6,3,2


def test_game_two_face():
    """Test that the game results API provides correct information."""
    game = setup_test_game_with_mock_deck_specified(create_predetermined_deck_two_face())
    ...
    assert "High Card" in winning_hand[0].hand_name
    assert "Ten High" in winning_hand[0].hand_description
