"""Minimal end-to-end test for archie."""
import pytest
...
def setup_test_game():
    ...
    rules = load_rules_from_file('archie')
    game = Game(
        rules=rules,
...
def test_archie_minimal_flow():
    """Test minimal flow for archie from start to showdown."""
    ...
    assert winning_hand[0].hand_description == 'Jack High'  # Updated from run

    # Check winning hands list
    assert len(results.winning_hands) == 6  # Updated from run
