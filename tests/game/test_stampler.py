"""Minimal end-to-end test for stampler."""
import pytest
...
def setup_test_game():
    ...
    rules = load_rules_from_file('stampler')
    game = Game(
        rules=rules,
...
def test_stampler_minimal_flow():
    ...
    assert hole_pot.winners[0] in ['BTN']

    # Check winning hand
    winning_player = hole_pot.winners[0]
    winning_hand = results.hands[winning_player]
    assert winning_hand[1].hand_name == "Highest Rank Spade"
    assert winning_hand[1].hand_description == "Highest Rank Spade (Qs)"