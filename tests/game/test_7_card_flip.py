...
def test_game_bringin():
    ...
    assert game.table.players["p1"].hand.cards[1].visibility == Visibility.FACE_UP
    assert game.table.players["p2"].hand.cards[0].visibility == Visibility.FACE_UP
    assert game.table.players["p2"].hand.cards[1].visibility == Visibility.FACE_UP
    assert game.table.players["p3"].hand.cards[0].visibility == Visibility.FACE_UP
    assert game.table.players["p3"].hand.cards[1].visibility == Visibility.FACE_UP

    # Step 3: Post Bring-In
    game._next_step()
    assert game.current_step == 3
    assert game.state == GameState.BETTING
    assert game.current_player.id == "p3"  # Charlie has pair of jacks - highest hand
    ...
    # also test total_bets and total_antes from Pot
    assert game.betting.pot.total_bets == {'round_2_p1': 1, 'round_2_p2': 1, 'round_2_p3': 4}
    assert game.betting.pot.total_antes == {'round_2_p1': 1, 'round_2_p2': 1, 'round_2_p3': 1}    
    ...
    # Step 5: Deal Fifth Street (fifth card, face up)
    game._next_step()
    ...
    assert game.current_player.id == "p3"  # Bob (Kd,Qc)
    valid_actions = game.get_valid_actions("p3")
    assert (PlayerAction.FOLD, None, None) in valid_actions
    ...
    # Charlie bets $20
    result = game.player_action("p3", PlayerAction.BET, 20)
    assert result.success
    assert game.betting.current_bet == 20
    assert game.table.players["p3"].stack == 459  # 479 - 20
    assert game.betting.get_main_pot_amount() == 83  # 63 + 20

    # Alice calls $20
    assert game.current_player.id == "p1"  # Bob next
    valid_actions = game.get_valid_actions("p1")
    assert (PlayerAction.FOLD, None, None) in valid_actions
    assert (PlayerAction.CALL, 20, 20) in valid_actions
    result = game.player_action("p1", PlayerAction.CALL, 20)
    ...
    assert game.state == GameState.BETTING
    # Upcards: Alice: Ah,Kh,Jh,9s; Bob: Qd,Kc,Td,7s; Charlie: Js,Jd,Th,6s
    # Charlie has highest (Js,Jd), starts betting
    assert game.current_player.id == "p3"  # Charlie
    valid_actions = game.get_valid_actions("p3")
    assert (PlayerAction.FOLD, None, None) in valid_actions
