...
def setup_test_game():
    ...
    rules = load_rules_from_file('scrotum_8')
    game = Game(
        rules=rules,
...
def test_sack_8_raise_in_group():
    game = setup_test_game()
    game.start_hand()
    # Step 0: Post Blinds
    assert game.current_step == 0
    ...
    cards_to_discard = hand.get_cards()[:4]
    game.player_action('BTN', PlayerAction.DISCARD, cards=cards_to_discard)
    assert len(hand.get_cards()) == initial_count - 4

    # SB: Call 10, Discard 2
    assert game.current_player.id == 'SB' 
    assert game.action_handler.current_substep == 0    
    assert game.current_step == 2
    assert game.state == GameState.BETTING
    ...
    game.player_action('BB', PlayerAction.RAISE, 20)  # BB raising to 20
    # and discard
    assert game.current_step == 2
    assert game.state == GameState.DRAWING
    assert game.action_handler.current_substep == 1    
    ...
    assert game.current_player.id == 'BTN'  # BTN first pre-flop
    actions = game.get_valid_actions('BTN')
    assert (PlayerAction.CALL, 20, 20) in actions
    game.player_action('BTN', PlayerAction.CALL, 20)    

    # SB: Call 20 (additional 10)
    assert game.current_player.id == 'SB' 
    assert game.action_handler.current_substep == 0    
    assert game.current_step == 2
    assert game.state == GameState.BETTING
    actions = game.get_valid_actions('SB')
    assert (PlayerAction.CALL, 20, 20) in actions
    game.player_action('SB', PlayerAction.CALL, 20)  # SB completes to 10

    # we are done - BB doesn't need to act again
...
def test_sack_8_showdown():
    ...
    assert results.pots[0].amount == 15  # High Hand pot
    assert results.pots[1].amount == 15  # Low Hand pot
    ...
    # low pot
    low_pot = results.pots[1]
    assert low_pot.pot_type == 'main'  # Updated from run
    assert low_pot.split == False  # Updated from run
    assert len(low_pot.winners) == 1  # Updated from run
    assert sorted(low_pot.winners) == ['SB']  # Updated from run

    # Check winning hand
    winning_player = low_pot.winners[0]
    winning_hand = results.hands[winning_player]
    assert winning_hand[1].hand_name == 'High Card'  # Updated from run
    assert winning_hand[1].hand_description == 'Six High'  # Updated from run

    # Check winning hands list
    assert len(results.winning_hands) == 2  # Updated from run
