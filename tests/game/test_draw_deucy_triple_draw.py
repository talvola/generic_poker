"""Minimal end-to-end test for draw_deucey___triple_draw."""
import pytest
...
def setup_test_game():
    """Setup a 3-player draw_deucey___triple_draw game with a mock deck."""
    rules = load_rules_from_file('draw_deucy_triple_draw')
    game = Game(
        rules=rules,
...
def test_draw_deucey___triple_draw_minimal_flow():
    """Test minimal flow for draw_deucey___triple_draw from start to showdown."""
    ...
    game._next_step()  # post-draw_bet_#1
    ...
    game._next_step()  # post-draw_bet_#2
    ...
    game._next_step()  # post-draw_bet_#3
    ...
    assert results.pots[0].amount == 15  # High pot
    assert results.pots[1].amount == 15  # 2-7 Lowball pot
    ...
    assert sorted(main_pot.winners) == ['SB']  # Updated from run
    ...
    assert winning_hand[0].hand_name == 'High Card'  # Updated from run
    assert winning_hand[0].hand_description == 'Ace High'  # Updated from run

    # Check winning hands list
    assert len(results.winning_hands) == 2  # Updated from run
    assert results.winning_hands[0].player_id == 'SB'  # Updated from run
