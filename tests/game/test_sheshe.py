...
def create_predetermined_deck():
    ...
    cards = [
        ...
        Card(Rank.TWO, Suit.SPADES), #FLOP
        Card(Rank.TWO, Suit.DIAMONDS), #FLOP
        Card(Rank.TWO, Suit.HEARTS), #FLOP
        Card(Rank.THREE, Suit.SPADES), #TURN
        Card(Rank.FOUR, Suit.SPADES), #RIVER
...
def setup_test_game_with_mock_deck():
    ...
    rules = load_rules_from_file('sheshe')
...
def test_game_results_showdown():
    ...
    # Step 2: Separate Cards into HE and SHE hands
    ...
    assert any(action[0] == PlayerAction.SEPARATE and action[1] == 5 and action[2] == 5 for action in valid_actions)
    ...
    assert str(game.table.community_cards["default"][0]) == "2s"  # Flop 1
    assert str(game.table.community_cards["default"][1]) == "2d"  # Flop 2
    assert str(game.table.community_cards["default"][2]) == "2h"  # Flop 3
    ...
    # Check community cards
    assert str(game.table.community_cards["default"][3]) == "3s"  # Turn
    ...
    # Check community cards
    assert str(game.table.community_cards["default"][4]) == "4s"  # River
    ...
    # Get the pots
    board_1_pot = next((pot for pot in results.pots if pot.hand_type == "Hold'em"), None)
    board_2_pot = next((pot for pot in results.pots if pot.hand_type == "Super Hold'em"), None)
    ...
    winning_b1 = next(hand for hand in results.winning_hands if hand.hand_type == "Hold'em")
    assert winning_b1.player_id == "BB", f"Expected BB as board 1 hand winner, got {winning_b1.player_id}"
    assert "Full House" in winning_b1.hand_name, f"Expected 'Full House' in winning high hand, got {winning_b1.hand_name}"
    assert "Twos over Jacks" in winning_b1.hand_description, f"Expected 'Twos over Jacks' in winning high hand description, got {winning_b1.hand_description}"

    winning_b2 = next(hand for hand in results.winning_hands if hand.hand_type == "Super Hold'em")
    ...
    assert "Twos over Queens" in winning_b2.hand_description, f"Expected 'Twos over Queens' in winning high hand description, got {winning_b2.hand_description}"
