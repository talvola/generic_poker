"""Minimal end-to-end test for five_card_draw."""
import pytest
...
def create_predetermined_deck():
    ...
    named_cards = [
        ...
        Card(Rank.THREE, Suit.HEARTS), Card(Rank.KING, Suit.CLUBS), Card(Rank.JACK, Suit.DIAMONDS),
        Card(Rank.NINE, Suit.HEARTS), Card(Rank.KING, Suit.DIAMONDS), Card(Rank.TEN, Suit.SPADES),
        Card(Rank.NINE, Suit.CLUBS), Card(Rank.KING, Suit.SPADES), Card(Rank.JACK, Suit.HEARTS),
        Card(Rank.NINE, Suit.DIAMONDS), Card(Rank.TEN, Suit.HEARTS), Card(Rank.NINE, Suit.SPADES),
        # one card for SB, BB, BTN
        Card(Rank.FOUR, Suit.SPADES), Card(Rank.TWO, Suit.DIAMONDS), Card(Rank.SIX, Suit.HEARTS),
        Card(Rank.FIVE, Suit.DIAMONDS), Card(Rank.SIX, Suit.CLUBS), Card(Rank.ACE, Suit.SPADES),
    ]
    return MockDeck(named_cards)

def setup_test_game():
    """Setup a 3-player five_card_draw game with a mock deck."""
    rules = load_rules_from_file('quick_quads')
    game = Game(
        rules=rules,
...
def test_five_card_draw_minimal_flow():
    """Test minimal flow for five_card_draw from start to showdown."""
    ...
    assert any(a[0] == PlayerAction.DRAW and a[1] == 0 and a[2] == 5 for a in actions)
    ...
    assert winning_hand[0].hand_name == 'Four of a Kind'  # Updated from run
    assert winning_hand[0].hand_description == 'Quick Quads Nines'  # Updated from run
