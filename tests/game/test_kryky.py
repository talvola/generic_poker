...
def test_kryky_card_dealing_order():
    ...
    print(
        f"Alice cards: {[str(c) + ('(up)' if c.visibility == Visibility.FACE_UP else '(down)') for c in alice_cards]}"
    )
    print(f"Bob cards: {[str(c) + ('(up)' if c.visibility == Visibility.FACE_UP else '(down)') for c in bob_cards]}")
    print(
        f"Charlie cards: {[str(c) + ('(up)' if c.visibility == Visibility.FACE_UP else '(down)') for c in charlie_cards]}"
    )
...
def test_kryky_full_hand_with_showdown():
    ...
    result = game.player_action(bring_in_player, PlayerAction.COMPLETE, 10)
