"""Minimal end-to-end test for omaha_hi_hi."""
import pytest
...
def setup_test_game():
    """Setup a 3-player omaha_hi_hi game with a mock deck."""
    rules = load_rules_from_file('omaha_hi_hi')
    game = Game(
        rules=rules,
...
def test_omaha_hi_hi_minimal_flow():
    """Test minimal flow for omaha_hi_hi from start to showdown."""
    ...
    for pid in ['BTN', 'SB', 'BB']:
        assert len(game.table.players[pid].hand.get_cards()) == 4 + initial_hole_cards[pid]
        initial_hole_cards[pid] += 4
    print(f'\nStep 1 - Player Hands:')
    for pid in ['BTN', 'SB', 'BB']:
    ...
    assert high_pot.amount == results.total_pot // 2  
    assert high_pot.pot_type == 'main'
    assert not high_pot.split  
    assert len(high_pot.winners) == 1  
    assert high_pot.winners[0] in ['BB']

    # Check winning hand
    winning_player = high_pot.winners[0]
    winning_hand = results.hands[winning_player]
    assert winning_hand[0].hand_name == "Three of a Kind"  
    assert winning_hand[0].hand_description == "Three Tens"  
    # Check winning hands list
    assert len(results.winning_hands) == 2  

    # Check pot details
    hole_pot = results.pots[1]
    assert hole_pot.amount == results.total_pot // 2  
    assert hole_pot.pot_type == 'main'
    assert not hole_pot.split  
    assert len(hole_pot.winners) == 1  
    assert hole_pot.winners[0] in ['BB']

    # Check winning hand
    winning_player = hole_pot.winners[0]
    winning_hand = results.hands[winning_player]
    assert winning_hand[1].hand_name == "Pair"  
    assert winning_hand[1].hand_description == "Pair of Jacks"  