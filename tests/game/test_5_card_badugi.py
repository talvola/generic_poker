"""Minimal end-to-end test for 5_card_badugi."""
import pytest
...
def setup_test_game():
    """Setup a 3-player 5_card_badugi game with a mock deck."""
    rules = load_rules_from_file('5_card_badugi')
    game = Game(
        rules=rules,
...
def test_5_card_badugi_minimal_flow():
    """Test minimal flow for 5_card_badugi from start to showdown."""
    ...
    assert winning_hand[0].hand_name == 'Two-Card Hand'  # Updated from run
    assert winning_hand[0].hand_description == 'Two-Card Hand'  # Updated from run
