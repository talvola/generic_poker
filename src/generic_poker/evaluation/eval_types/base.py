...
from generic_poker.evaluation.constants import HAND_SIZES, PADDED_TYPES, RANK_ONLY_TYPES, RANK_ORDERS, SUIT_ORDER
...
class BaseEvaluator(ABC):
    ...
    def _cards_to_string(self, cards: list[Card | str]) -> str:
        """
        ...
        if self.rank_only:
            return "".join(
                card if isinstance(card, str) else getattr(getattr(card, "rank", None), "value", "X") for card in cards
            )
        else:
            return "".join(card if isinstance(card, str) else f"{card.rank.value}{card.suit.value}" for card in cards)
    ...
    def _validate_hand_size(self, cards: list[Card]) -> None:
        ...
        if len(cards) != self.required_size and not self.padding_required:
            raise ValueError(f"{self.eval_type} evaluation requires exactly {self.required_size} cards")

    def _sort_cards(self, cards: list[Card | str]) -> list[Card | str]:
        """
        ...
        def get_sort_key(item: Card | str) -> tuple:
    ...
    @abstractmethod
    def get_sample_hand(self, rank: int, ordered_rank: int) -> HandRanking:
        pass

    @abstractmethod
    def evaluate(self, cards: list[Card], wild_cards: list[Card] | None = None) -> HandRanking | None:
