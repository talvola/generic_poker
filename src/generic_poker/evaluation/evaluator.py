...
class HandResult:
    ...
    cards_used: list[Card] | None = None
    ...
    def from_ranking(cls, ranking: HandRanking) -> "HandResult":
        ...
        return cls(rank=ranking.rank, ordered_rank=ranking.ordered_rank, description=ranking.description)
...
class HandEvaluator:
    ...
    def evaluate_hand(
        self, cards: list[Card], eval_type: EvaluationType, qualifier: list[int] | None = None
    ) -> HandResult:
        """
    ...
    def get_sample_hand(self, eval_type: EvaluationType, rank: int, ordered_rank: int) -> list[Card]:
        """
    ...
    def sort_cards(self, cards: list[Card], eval_type: EvaluationType | None = None) -> list[Card]:
        """
    ...
    def compare_hands(
        self, hand1: list[Card], hand2: list[Card], eval_type: EvaluationType, qualifier: list[int] | None = None
    ) -> int:
        """
    ...
    def _meets_qualifier(self, result: HandResult | None, qualifier: list[int]) -> bool:
    ...
    def compare_hands_with_offset(
        ...
        qualifier1: list[int] | None = None,
        qualifier2: list[int] | None = None,
    ) -> int:
        """
        ...
        if comparison_file.exists():
            ...
            equivalent_rank = self._get_equivalent_rank(
                comparison_table, smaller_result.rank, smaller_result.ordered_rank
            )
            ...
            if equivalent_rank is not None:
                ...
                if larger_result.rank < mapped_rank:
                    ...
                    larger_ordered_rank = (
                        larger_result.ordered_rank if larger_result.ordered_rank is not None else float("inf")
                    )
    ...
    def _get_equivalent_rank(
        self, table: list[dict[str, str]], rank: int, ordered_rank: int | None
        ...
        for row in table:
            if int(row["two_card_rank"]) == rank and int(row["two_card_ordered_rank"]) == ordered_rank:
