...
from generic_poker.evaluation.constants import BASE_RANKS, HAND_SIZES
...
from generic_poker.game.game_result import GameResult, HandResult, PotResult
...
class ShowdownManager:
    ...
    def _handle_player_specific_showdown(self, active_players: list) -> GameResult:
        ...
        if all_hand_results:
            ...
            if best_players:
                ...
                self._award_full_pot_to_winners(best_players, original_main_pot, original_side_pots, pot_results)
    ...
    def _award_full_pot_to_winners(
        self, winners: list, original_main_pot: int, original_side_pots: list[int], pot_results: list
    ) -> None:
    ...
    def _evaluate_config_hands(
        self, active_players: list, config: dict, eval_type: EvaluationType, hand_results: dict
    ) -> dict:
    ...
    def _record_special_winning_hands(
        ...
        for player in best_players:
            ...
            hand_description = (
                f"{hand_name} ({', '.join(str(c) for c in qualifying_cards)})" if qualifying_cards else hand_name
            )
    ...
    def _handle_global_default_action(
        ...
        if action_type == "split_pot":
            self._handle_global_split_pot(
                active_players, original_main_pot, original_side_pots, pot_results, hand_results, winning_hands
            )
        elif action_type == "best_hand":
            self._handle_global_best_hand(
                action, active_players, original_main_pot, original_side_pots, pot_results, hand_results, winning_hands
            )
    ...
    def _handle_global_best_hand(
        ...
        for alt_config in alternate_configs:
            ...
            alt_winners, had_alt_winners = self._award_alternate_pots(
                active_players, alt_results, alt_eval_type, alt_config, original_main_pot, original_side_pots
            )
    ...
    def _create_final_game_result(
        self, pot_results: list, hand_results: dict, winning_hands: list, total_pot: int
    ) -> GameResult:
    ...
    def _find_hand_with_subset_requirements(
        ...
        for hole_combo in hole_combos:
            ...
            community_combinations = self._generate_subset_combinations(community_cards, subset_requirements)
    ...
    def _generate_subset_combinations(
        self, community_cards: dict[str, list[Card]], subset_requirements: list[dict[str, Any]]
    ) -> list[list[Card]]:
        ...
        def generate_combinations_recursive(
            requirements_index: int, current_combination: list[Card]
        ) -> list[list[Card]]:
            ...
            if len(available_cards) < count and required:
                logger.warning(
                    f"Not enough cards in required subset '{subset_name}': need {count}, have {len(available_cards)}"
                )
            ...
            for subset_combo in subset_combos:
                ...
                combinations.extend(generate_combinations_recursive(requirements_index + 1, new_combination))
    ...
    def handle_fold_win(self, active_players: list[Player]) -> GameResult:
        ...
        return GameResult(pots=[pot_result], hands=hand_results, winning_hands=winning_hands, is_complete=True)
    ...
    def _handle_showdown_with_declare(self) -> GameResult:
        ...
        if len(best_hand_configs) != 2 or showdown_rules.declaration_mode != "declare":
            logger.error(
                "Declarations only supported for Hi-Lo games with two configurations and declaration_mode='declare'"
            )
            return GameResult(pots=[], hands={}, winning_hands=[], is_complete=True)
        ...
        side_pots_eligible = [
            set(self.betting.get_side_pot_eligible_players(i)) for i in range(self.betting.get_side_pot_count())
        ]
        ...
        for pot_index in [-1] + list(range(self.betting.get_side_pot_count())):
            ...
            # Get declarations
            declarations = {pid: self.declarations.get(pid, {}).get(pot_index) for pid in eligible_ids}
            ...
            high_eligible = [pid for pid in eligible_ids if declarations.get(pid) in ["high", "high_low"]]
            low_eligible = [pid for pid in eligible_ids if declarations.get(pid) in ["low", "high_low"]]
            ...
            if high_eligible and high_results:
                high_hands = {
                    pid: high_results[pid] for pid in high_eligible if pid in high_results and high_results[pid]
                }
                if high_hands:
            ...
            if low_eligible and low_results:
                low_hands = {pid: low_results[pid] for pid in low_eligible if pid in low_results and low_results[pid]}
                if low_hands:
            ...
            for pid in H:
                ...
                if high_low_eligible[pid] and (decl == "high" or decl == "high_low"):
            ...
            for pid in L:
                decl = declarations.get(pid)
                if high_low_eligible[pid] and (decl == "low" or decl == "high_low"):
    ...
    def _evaluate_hands_for_config(
        self, players: list[Player], hand_config: dict, eval_type: EvaluationType
    ) -> dict[str, HandResult]:
        """
    ...
    def _award_pots_for_config(
        ...
        qualifier = hand_config.get("qualifier")
        ...
        for i in range(len(original_side_pots)):
            ...
            for player in eligible_players:
                ...
                if qualifier:
                    ...
                    if (
                        not result
                        or (result.rank > qualifier[0])
                        or (result.rank == qualifier[0] and result.ordered_rank > qualifier[1])
                    ):
                        continue
            ...
            if qualified_players:
                ...
                if winners:
                    ...
                    if len(self.rules.showdown.best_hand) == 2:
                        ...
                        elif "Omaha" in self.rules.showdown.best_hand[other_config_idx].get(
                            "name", ""
                        ) and "Draw" in self.rules.showdown.best_hand[current_config_idx].get("name", ""):
                            gets_odd_chip = True
                        elif "Omaha" in self.rules.showdown.best_hand[current_config_idx].get(
                            "name", ""
                        ) and "Draw" in self.rules.showdown.best_hand[other_config_idx].get("name", ""):
                            gets_odd_chip = False

                        # Rule 4: Sohe - Omaha hand gets odd chip
                        elif "Omaha" in self.rules.showdown.best_hand[current_config_idx].get(
                            "name", ""
                        ) and "Hold'em" in self.rules.showdown.best_hand[other_config_idx].get("name", ""):
                            gets_odd_chip = True
                        elif "Omaha" in self.rules.showdown.best_hand[other_config_idx].get(
                            "name", ""
                        ) and "Hold'em" in self.rules.showdown.best_hand[current_config_idx].get("name", ""):
                            gets_odd_chip = False

                        # Fallback to card count as a tiebreaker
                        else:
                            gets_odd_chip = card_counts.get(current_config_idx, 0) > card_counts.get(
                                other_config_idx, 0
                            )
                        ...
                        logger.debug(
                            f"Odd chip decision for {hand_config.get('name')} - "
        ...
        # Award main pot
        eligible_players = [player_dict[pid] for pid in player_dict]
    ...
    def _find_winners(
        ...
        for player in players[1:]:
            ...
            if current_classification_rank < best_classification_rank:
                ...
                comparison = evaluator.compare_hands(current_hand_result.cards, best_hand_result.cards, eval_type)
    ...
    def _handle_zero_cards_case(self, showdown_rules: dict, eval_type: EvaluationType) -> bool:
        ...
        return "zeroCardsPipValue" in showdown_rules and eval_type.startswith("low_pip")
    ...
    def _find_hand_with_combinations(
        ...
        for combo in showdown_rules["combinations"]:
            ...
            comm_combos = (
                [tuple()] if required_community == 0 else list(itertools.combinations(comm_cards, required_community))
            )
        ...
        if best_hand:
            ...
            logger.warning("No valid hand combinations found")
            return [], []
    ...
    def _find_hand_with_hole_card_options(
        self, hole_cards: list[Card], comm_cards: list[Card], showdown_rules: dict, eval_type: EvaluationType
    ) -> tuple[list[Card], list[Card]]:
    ...
    def _find_hand_with_hole_and_community(
        ...
        hole_combos = (
            [tuple()]
            if required_hole == 0
            else list(itertools.combinations(hole_cards, min(len(hole_cards), required_hole)))
        )
        community_combos = (
            [tuple()] if required_community == 0 else list(itertools.combinations(comm_cards, required_community))
        )
        ...
        if required_community > 0:
            ...
            if "community_subset" in rule:
                ...
                if len(subset_cards) < required_community:
                    logger.warning(
                        f"Not enough cards in subset '{subset_name}': need {required_community}, have {len(subset_cards)}"
                    )
                    return [], []
            ...
            # Check for multiple subsets
                ...
                if len(comm_cards) < required_community:
                    logger.warning(
                        f"Not enough cards in subsets {subset_names}: need {required_community}, have {len(comm_cards)}"
                    )
                    return [], []
            ...
            # Fallback to default community cards
                ...
                if len(default_cards) < required_community:
                    logger.warning(
                        f"Not enough default community cards: need {required_community}, have {len(default_cards)}"
                    )
                    return [], []
    ...
    def _find_best_hand_for_player(
        self, player: Player, community_cards: dict[str, list[Card]], showdown_rules: dict, eval_type: EvaluationType
    ) -> tuple[list[Card], list[Card]]:
        """
        ...
        if "communitySubsetRequirements" in showdown_rules:
            return self._find_hand_with_subset_requirements(hole_cards, community_cards, showdown_rules, eval_type)
        ...
        if "combinations" in showdown_rules:
            ...
            return self._find_hand_with_combinations(hole_cards, community_cards, showdown_rules, eval_type)

        if "communityCardSelectCombinations" in showdown_rules:
            return self._find_hand_with_select_combinations(hole_cards, community_cards, showdown_rules, eval_type)

        if "anyCards" in showdown_rules:
            return self._find_hand_with_any_cards(hole_cards, comm_cards, player, showdown_rules, eval_type)

        if "holeCards" in showdown_rules:
            if isinstance(showdown_rules["holeCards"], list):
                return self._find_hand_with_hole_card_options(hole_cards, comm_cards, showdown_rules, eval_type)
            ...
            return self._find_hand_with_hole_and_community(hole_cards, comm_cards, player, showdown_rules, eval_type)

        if "playerHandSize" in showdown_rules:
            return self._find_hand_with_player_hand_size(hole_cards, community_cards, showdown_rules, eval_type)
    ...
    def _get_community_cards_for_player(
        self, player: Player, community_cards: dict[str, list[Card]], showdown_rules: dict
    ) -> list[Card]:
        ...
        for rule in wild_rules:
            ...
            if role == "conditional" and "condition" in rule:
                ...
                for card in all_cards:
                    ...
                    if (
                        rule_type == "joker"
                        and card.rank == Rank.JOKER
                        or rule_type == "rank"
                        and card.rank == Rank(rule.get("rank", "R"))
                    ):
                        pass  # matches
                    else:
                        continue
    ...
    def _find_best_players_for_special(self, active_players, criterion, suit, from_source, subsets):
        ...
        if suit == "river_card_suit":
            try:
                ...
                river_card.suit.name.lower()  # For logging/description
            except IndexError:
        ...
        logger.debug(
            f"Best players for {suit} {criterion}: {[p.id for p in best_players]} with cards: {best_cards_dict}"
        )
        ...
        if main_pots:
            ...
            if additional_main > 0:
                for pot in main_pots:
                    ...
                    logger.info(
                        f"Redistributed ${additional_main} to {[p.name for p in winners]} (no qualifying low hand)"
                    )
        ...
        for idx, pots in side_pots.items():
            if idx < len(original_side_pots):
                ...
                for pot in pots:
                    ...
                    logger.info(
                        f"Redistributed ${additional_side} to {[p.name for p in winners]} for side pot {idx} (no qualifying low hand)"
                    )

    def _redistribute_exact_pot(
        self, original_main_pot: int, original_side_pots: list[int], awarded_pot_results: list[PotResult]
    ) -> None:
        """
