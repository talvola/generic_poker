...
from generic_poker.config.loader import ForcedBets, GameActionType, GameRules
from generic_poker.core.card import Card, Rank, Visibility, WildType
...
from generic_poker.game.table import Player, Position, Table
...
class Game:
    ...
    def __init__(
        ...
        mandatory_straddle: int | None = None,
        ...
        if structure not in rules.betting_structures:
            raise ValueError(f"Betting structure {structure} not allowed for {rules.game}")
        ...
        self.current_player: Player | None = None  # player to act
    ...
    def get_game_description(self) -> str:
        ...
        from generic_poker.game.betting import NoLimitBettingManager, PotLimitBettingManager
    ...
    def add_player(self, player_id: str, name: str, buyin: int, preferred_seat: int | None = None) -> None:
    ...
    def player_action(
        ...
        if result.advance_step and self.auto_progress and self.state != GameState.COMPLETE:
            ...
            for attr in [
                "current_discard_config",
                "current_draw_config",
                "current_separate_config",
        ...
        if step.action_type == GameActionType.GROUPED:
            ...
            if "bet" in first_subaction:
                ...
                if bet_config.get("type") in ["antes", "blinds", "bring-in"]:
                    ...
                    logger.debug(
                        f"Starting betting round: {step.name} with new_round({(self.betting.betting_round == 0)})"
                    )
    ...
    def _handle_deal(self, config: dict[str, Any], player_id=None) -> None:
        ...
        for card_config in config["cards"]:
            ...
            if protection_option and location == "player":
                ...
                should_continue = self._handle_standard_deal(
                    card_config, location, player_id, wild_card_rules, conditional_state, num_cards
                )
    ...
    def _handle_protected_deal(self, card_config: dict, player_id: str | None, wild_card_rules: list) -> None:
        ...
        if conditional_state and "state" not in card_config:
            ...
            if location == "player":
                if player_id:
                    ...
                    if wild_card_rules:
                        for _active_player_id, cards in cards_dealt_dict.items():
                            for card in cards:
    ...
    def _get_players_to_deal(self, player_id: str | None) -> list[Player]:
            ...
            if len(active_players) >= 2:
                ...
                sb_idx = next(
                    (
                        ...
                        if p.position and p.position.has_position(Position.SMALL_BLIND)
                    ),
                    None,
                )
    ...
    def get_effective_forced_bets(game_instance, forced_bets: ForcedBets) -> dict[str, Any]:
        ...
        if forced_bets.conditionalOrders is not None:
            ...
            return {"style": forced_bets.style, "rule": forced_bets.rule, "bringInEval": forced_bets.bringInEval}
    ...
    def handle_forced_bets(self, bet_type: str):
        ...
        if bet_type == "antes":
            ...
            if self.rules.betting_order.initial == "dealer":
                ...
                blind_player = next(
                    (p for p in positions if p.position and p.position.has_position(Position.BUTTON)), None
                )
                ...
                sb_player = next(
                    (p for p in positions if p.position and p.position.has_position(Position.SMALL_BLIND)), None
                )
                bb_player = next(
                    (p for p in positions if p.position and p.position.has_position(Position.BIG_BLIND)), None
                )
        ...
        logger.debug("Starting betting round with new_round(True)")
    ...
    def next_player(self, round_start: bool = False) -> Player | None:
        ...
        current_step_config = (
            self.rules.gameplay[self.current_step] if self.current_step < len(self.rules.gameplay) else None
        )
                ...
                if order_type == "dealer":
                    ...
                    if self.betting.last_actor_id:
                        ...
                        if last_actor and last_actor.is_active:
                            ...
                            try:
                                last_actor_idx = next(
                                    i for i, p in enumerate(players) if p.id == self.betting.last_actor_id
                                )
