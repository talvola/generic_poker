...
class Seat:
    ...
    reserved_for: str | None = None  # Player ID if reserved
...
class TableLayout:
    ...
    def get_player_seat(self, player_id: str) -> int | None:
    ...
    def remove_player_from_seat(self, seat_number: int) -> Player | None:
...
class Table:
    ...
    def add_player(self, player_id: str, name: str, buyin: int, preferred_seat: int | None = None) -> int:
        """
        ...
        if buyin < self.min_buyin or buyin > self.max_buyin:
            raise ValueError(f"Buy-in must be between {self.min_buyin} and {self.max_buyin}")
    ...
    def get_position_order(self, include_inactive: bool = True) -> list[Player]:
        ...
        ordered_seats = occupied_seats[button_idx:] + occupied_seats[:button_idx]

        # Get players in order
        players = []
        for seat_num in ordered_seats:
            seat = self.layout.seats[seat_num]
            if seat.player and (include_inactive or seat.player.is_active):
                players.append(seat.player)
    ...
    def get_player_in_seat(self, seat_number: int) -> Player | None:
    ...
    def get_player_seat_number(self, player_id: str) -> int | None:
        """Get the seat number for a specific player."""
        return self.layout.get_player_seat(player_id)

    # Maintain backward compatibility with existing methods
    def get_player_after_big_blind(self) -> Player | None:
        """Return the first active player after the big blind."""
        players = self.get_position_order()
        bb_idx = next(
            (i for i, p in enumerate(players) if p.position and p.position.has_position(Position.BIG_BLIND)), -1
        )
        if bb_idx == -1 or len(players) <= bb_idx + 1:
            return None
        return players[bb_idx + 1]

    def get_next_active_player(self, start_position: int) -> Player | None:
        """
    ...
    def get_next_player_after(self, current_player_id: str) -> Player | None:
        """
    ...
    def get_bring_in_player(self, bring_in_amount: int) -> Player | None:
        ...
        bring_in_rule = (
            CardRule(self.rules.forced_bets.rule) if self.rules and self.rules.forced_bets.rule else CardRule.LOW_CARD
        )
    ...
    def get_player_with_best_hand(self, forced_bets: ForcedBets) -> Player | None:
        ...
        max_face_up = max(
            sum(1 for c in p.hand.get_cards() if c.visibility == Visibility.FACE_UP) for p in active_players
        )
        ...
        eval_type = BringInDeterminator._get_dynamic_eval_type(total_visible, bring_in_rule, forced_bets, self.rules)
        logger.debug(
            f"Evaluating best hand with {max_face_up} player + {len(visible_community)} community visible cards using {eval_type}"
        )
        ...
        def best_n_cards(cards, n):
            ...
            for combo in itertools.combinations(cards, n):
                ...
                if best is None or evaluator.compare_hands(combo_list, best, eval_type) > 0:
                    best = combo_list
        ...
        for player in active_players:
            player_visible = [c for c in player.hand.get_cards() if c.visibility == Visibility.FACE_UP]
            ...
            try:
                ...
                if best_hand is None or evaluator.compare_hands(player_hand, best_hand, eval_type) > 0:
    ...
    def _get_visible_community_cards_for_betting(self) -> list[Card]:
        ...
        if (
            self.rules
            and hasattr(self.rules, "showdown")
            and self.rules.showdown
            and hasattr(self.rules.showdown, "best_hand")
            and self.rules.showdown.best_hand
        ):
            ...
            for hand_config in self.rules.showdown.best_hand:
                if isinstance(hand_config, dict) and "community_subset" in hand_config:
                    ...
                    if subset_name in self.community_cards:
                        visible_cards = [
                            c for c in self.community_cards[subset_name] if c.visibility == Visibility.FACE_UP
                        ]
                        logger.debug(
                            f"Using community subset '{subset_name}' for betting order: {len(visible_cards)} cards"
                        )
    ...
    def get_player_to_act(self, round_start: bool = False) -> Player | None:
        """
    ...
    def deal_card_to_player(self, player_id: str, face_up: bool = False, subset: str = "default") -> Card | None:
    ...
    def deal_community_cards(self, num_cards: int, subsets: list[str] = None, face_up: bool = True) -> list[Card]:
    ...
    def expose_community_cards(self, subset: str = "default", indices: list[int] | None = None) -> None:
