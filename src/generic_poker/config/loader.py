...
class BettingOrder:
    ...
    initial: str
    subsequent: str | dict[str, Any]  # Now supports both string and conditional object
...
class GameStep:
    ...
    action_config: dict[str, Any] | list[dict[str, Any]]  # Supports single or grouped actions
    conditional_state: dict[str, Any] | None = None  # Add conditional_state field with default None
...
class GameRules:
    ...
    category: str = ""
    community_card_layout: dict[str, Any] | None = None
        ...
        if "conditionalOrders" in forced_bets_data:
            ...
            for i, cond_order in enumerate(conditional_orders):
                ...
                if style == "bring-in" and not forced_bet.get("rule"):
                    logger.warning(
                        f"Missing 'rule' for bring-in style in conditional order {i}, defaulting to 'low card'"
                    )
            ...
            return ForcedBets(conditionalOrders=conditional_orders, default=default_config)
    ...
    def from_json(cls, json_str: str) -> "GameRules":
                ...
                if "conditionalOrders" not in subsequent or "default" not in subsequent:
                    ...
                    for cond_order in subsequent["conditionalOrders"]:
                        ...
                        if cond_order["order"] not in valid_orders:
                            logger.warning(
                                f"Invalid conditional order '{cond_order['order']}', defaulting based on forcedBets"
                            )
                            subsequent = None
                            break
                    ...
                    if subsequent and subsequent["default"] not in valid_orders:
                        logger.warning(
                            f"Invalid default order '{subsequent['default']}', defaulting based on forcedBets"
                        )
                        subsequent = None
            else:
        ...
        betting_order = BettingOrder(initial=initial or default_initial, subsequent=subsequent or default_subsequent)
        ...
        for step in data["gamePlay"]:
            ...
            if "groupedActions" in step:
                ...
                for action in action_config:
                    if not any(key.lower() in action for key in GameActionType.__members__ if key != "GROUPED"):
            ...
            game_step = GameStep(name=step["name"], action_type=action_type, action_config=action_config)
    ...
    def validate(self) -> None:
        ...
        deck_sizes = {DeckType.STANDARD: 52, DeckType.SHORT_27_JA: 40, DeckType.SHORT_6A: 36, DeckType.SHORT_TA: 20}
    ...
    def _validate_gameplay_sequence(self) -> None:
        ...
        if total_dealt + community_dealt > self.deck_size:
            raise ValueError(f"Game requires {total_dealt + community_dealt} cards but deck only has {self.deck_size}")
