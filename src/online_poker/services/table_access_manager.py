...
class TableAccessManager:
        ...
        try:
            ...
            if as_spectator:
                ...
                access_record = TableAccess(
                    table_id=table_id, user_id=user_id, invite_code_used=invite_code, is_spectator=True
                )
    ...
    def get_table_players(table_id: str) -> list[dict[str, Any]]:
        ...
        try:
            access_records = (
                db.session.query(TableAccess)
                .filter(TableAccess.table_id == table_id, TableAccess.is_active == True)
                .order_by(TableAccess.seat_number.asc())
                .all()
            )
    ...
    @staticmethod
    def get_user_access(user_id: str, table_id: str) -> TableAccess | None:
    ...
    def get_ready_status(table_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"players": [], "player_count": 0, "all_ready": False, "ready_count": 0, "min_players": 2}
    ...
    def _cleanup_table_access(table_id: str) -> int:
        ...
        try:
            ...
            access_records = db.session.query(TableAccess).filter(TableAccess.table_id == table_id).all()
    ...
    def cleanup_inactive_access(timeout_minutes: int = 30) -> int:
        ...
        """
        try:
            inactive_records = db.session.query(TableAccess).filter(TableAccess.is_active == True).all()
