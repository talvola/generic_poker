...
class GameActionLogger:
    ...
    def log_forced_bet_action(
        self, table_id: str, step: int, game_state: str, player_name: str, bet_type: str, amount: int
    ) -> GameAction:
    ...
    def log_dealing(
        self, table_id: str, step: int, game_state: str, deal_type: str, details: dict = None
    ) -> GameAction:
    ...
    def log_phase_change(
        self, table_id: str, step: int, old_state: str, new_state: str, phase_name: str = None
    ) -> GameAction:
    ...
    def log_showdown_result(
        self, table_id: str, step: int, game_state: str, winners: list[dict], pot_amount: int, details: dict = None
    ) -> GameAction:
        ...
        if len(winners) == 1:
            winner = winners[0]
            message = (
                f"{winner['player_name']} wins ${pot_amount} with {winner.get('hand_description', 'the best hand')}"
            )
        else:
