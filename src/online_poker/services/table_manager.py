...
class TableManager:
    ...
    def get_public_tables() -> list[dict[str, Any]]:
        ...
        try:
            tables = (
                ...
                .filter(PokerTable.is_private == False)
    ...
    @staticmethod
    def get_table_by_id(table_id: str) -> PokerTable | None:
        """Get table by ID.
        ...
        """
        try:
            return db.session.query(PokerTable).filter(PokerTable.id == table_id).first()
        except Exception as e:
            current_app.logger.error(f"Failed to get table {table_id}: {e}")
            return None

    @staticmethod
    def get_table_by_invite_code(invite_code: str) -> PokerTable | None:
        ...
        try:
            return (
                db.session.query(PokerTable)
                .filter(PokerTable.invite_code == invite_code, PokerTable.is_private == True)
                .first()
            )
        except Exception as e:
    ...
    def can_access_table(
        table: PokerTable, user_id: str, invite_code: str | None = None, password: str | None = None
    ) -> tuple[bool, str]:
    ...
    @staticmethod
    def regenerate_invite_code(table_id: str, user_id: str) -> tuple[bool, str, str | None]:
        ...
        try:
            ...
            if success:
                current_app.logger.info(
                    f"Player {player_id} kicked from table {table_id} by {creator_id}. Reason: {reason}"
                )
    ...
    def transfer_host_privileges(table_id: str, current_creator_id: str, new_creator_id: str) -> tuple[bool, str]:
        ...
        try:
            ...
            current_app.logger.info(
                f"Host privileges transferred from {old_creator_id} to {new_creator_id} for table {table_id}"
            )
