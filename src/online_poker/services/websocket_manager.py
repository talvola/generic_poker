...
class WebSocketManager:
    ...
    def _register_handlers(self):
        ...
        def handle_connect(auth=None):
            ...
            try:
                ...
                if current_user.is_authenticated:
                    ...
                    emit(
                        "connected",
                        {"user_id": None, "username": "Guest", "timestamp": datetime.utcnow().isoformat() + "Z"},
                    )
        ...
        def handle_connect_to_table_room(data):
            ...
            try:
                ...
                if success:
                    ...
                    emit("table_joined", {"table_id": table_id, "timestamp": datetime.utcnow().isoformat() + "Z"})
                else:
        ...
        def handle_chat_message(data):
            ...
            try:
                ...
                chat_message = chat_service.send_message(table_id=table_uuid, user_id=user_uuid, message=message)
                ...
                emit(
                    "message_sent",
                    {"message_id": str(chat_message.id), "timestamp": chat_message.created_at.isoformat()},
                )
        ...
        def handle_player_action(data):
            ...
            try:
                ...
                success, message, result = session.process_player_action(
                    user_id, player_action, amount, cards=card_objects, declaration_data=declaration_data
                )
                ...
                if success:
                    ...
                    if action_lower == "fold":
                        ...
                        action_message = (
                            f"{current_user.username} calls ${amount}" if amount else f"{current_user.username} calls"
                        )
                        ...
                        action_message = (
                            f"{current_user.username}: {action} ${amount}"
                            if amount
                            else f"{current_user.username}: {action}"
                        )
        ...
        def handle_game_state_request(data):
            ...
            try:
                ...
                game_state = GameStateManager.generate_game_state_view(table_id, user_id, is_spectator)
        ...
        def handle_chat_history_request(data):
            ...
            try:
                ...
                emit("chat_history", {"table_id": table_id, "messages": chat_history})
            ...
            try:
                ...
                if action_type == "mute":
                    ...
                    unmute_action = chat_service.unmute_user(
                        table_id=table_id, target_user_id=target_user_id, moderator_user_id=user_id
                    )
                ...
                if success:
                    emit("moderation_result", {"success": True, "message": message})
        ...
        def handle_toggle_chat(data):
            ...
            try:
                ...
                emit(
                    "chat_toggled",
                    {"table_id": table_id, "enabled": enabled, "timestamp": datetime.utcnow().isoformat() + "Z"},
                )
    ...
    def broadcast_to_table(
        self, table_id: str, event: str, data: dict[str, Any], exclude_user: str | None = None
    ) -> None:
    ...
    def handle_table_disconnect(self, user_id: str, table_id: str) -> None:
        ...
        try:
            ...
            success, message = disconnect_manager.handle_player_disconnect(user_id, table_id, is_current_player)
    ...
    def handle_table_reconnect(self, user_id: str, table_id: str) -> bool:
        ...
        try:
            ...
            success, message, reconnect_info = disconnect_manager.handle_player_reconnect(user_id, table_id)
        ...
        try:
            ...
            for session_id in room_sessions:
                ...
                if user_id:
                    ...
                    if game_state:
                        self.socketio.emit(GameEvent.GAME_STATE_UPDATE, game_state.to_dict(), room=session_id)
        ...
        try:
            ...
            if not session:
                ...
                if not success or not session:
                    ...
                    self.broadcast_to_table(
                        table_id, GameEvent.ERROR, {"message": f"Failed to create game session: {message}"}
                    )
                    return
            ...
            if game and len(game.table.players) >= 2:
                ...
                self.broadcast_to_table(
                    table_id, GameEvent.ERROR, {"message": "Cannot start hand - not enough players with chips"}
                )
            ...
            for _ in range(max_iterations):
                ...
                if hasattr(game, "state") and game.state != GS.COMPLETE:
                    ...
                    while game.state != GS.COMPLETE:
                        if game.current_step >= len(game.rules.gameplay):
                            break
                        if game.state == GS.DEALING or game.state == GS.BETTING and game.current_player is None:
                            game._next_step()
                        else:
                            break
    ...
    def broadcast_ready_status(self, table_id: str) -> None:
        ...
        try:
            ...
            self.broadcast_to_table(
                table_id,
                GameEvent.READY_STATUS_UPDATE,
                {"table_id": table_id, "ready_status": ready_status, "timestamp": datetime.utcnow().isoformat() + "Z"},
            )
