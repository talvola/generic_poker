...
from ..models.chat import ChatFilter, ChatMessage, ChatModerationAction
...
class ChatService:
    ...
    def send_message(
        self, table_id: uuid.UUID, user_id: uuid.UUID, message: str, message_type: str = "chat"
    ) -> ChatMessage | None:
    ...
    def get_table_messages(
        self, table_id: uuid.UUID, limit: int = 50, include_system: bool = True
    ) -> list[ChatMessage]:
        ...
        """
        query = db.session.query(ChatMessage).filter(ChatMessage.table_id == table_id, ChatMessage.is_deleted == False)
    ...
    def mute_user(
        ...
        existing_mutes = (
            db.session.query(ChatModerationAction)
            .filter(
    ...
    def unmute_user(
        self, table_id: uuid.UUID, target_user_id: uuid.UUID, moderator_user_id: uuid.UUID | None = None
    ) -> ChatModerationAction | None:
    ...
    def get_moderation_actions(self, table_id: uuid.UUID, limit: int = 100) -> list[ChatModerationAction]:
        ...
        return (
            db.session.query(ChatModerationAction)
            .filter(ChatModerationAction.table_id == table_id)
            .order_by(ChatModerationAction.created_at.desc())
            .limit(limit)
            .all()
        )

    def _filter_message(self, message: str) -> tuple[str | None, bool]:
        ...
        for pattern in self._profanity_patterns:
            if re.search(pattern, message, re.IGNORECASE):
                ...
                filtered_message = re.sub(
                    pattern, lambda m: "*" * len(m.group()), filtered_message, flags=re.IGNORECASE
                )
                is_filtered = True
    ...
    def add_filter_word(
        ...
        chat_filter = ChatFilter(
            pattern=pattern, filter_type=filter_type, action=action, replacement=replacement, severity=severity
        )
