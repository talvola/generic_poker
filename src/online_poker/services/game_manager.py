...
from generic_poker.game.game import Game, GameState

from .game_action_logger import game_action_logger
from .simple_bot import SimpleBot, bot_manager
...
class GameManager:
    ...
    def get_game(self, table_id: str) -> Game | None:
        """Get game instance for a table."""
        return self.games.get(table_id)

    def add_player_to_game(
        self, table_id: str, player_id: str, username: str, stack: int, seat_number: int = None
    ) -> bool:
    ...
    def get_game_state(
        self, table_id: str, table_data: dict | None = None, current_user_id: str | None = None
    ) -> dict | None:
        ...
        try:
            ...
            if game.current_player and game.state == GameState.BETTING:
                try:
                    ...
                    print(
                        f"DEBUG: Game engine returned {len(actions)} actions for {game.current_player.name}: {[a[0].value for a in actions]}"
                    )
    ...
    def _get_dealer_position(self, game: Game, table_id: str) -> int | None:
    ...
    def _get_player_cards(self, player) -> list[dict]:
        ...
        try:
            if hasattr(player, "hand") and player.hand.cards:
                return [{"rank": str(card.rank.value), "suit": card.suit.value} for card in player.hand.cards]
            return []
        except Exception:
            return []
    ...
    def _format_valid_actions(self, actions: list[tuple]) -> list[dict]:
        ...
        for action_tuple in actions:
            ...
            formatted.append({"type": action_type.value.lower(), "min_amount": min_amount, "max_amount": max_amount})
        return formatted
    ...
    def process_bot_action(self, table_id: str) -> bool:
        ...
        try:
            ...
            print(
                f"DEBUG: Bot action - Player ID: {current_player_id}, Player Name: {player_name}, Action: {action_type}, Amount: {amount}"
            )
    ...
    def log_missed_forced_bets(self, table_id: str):
        ...
        try:
            ...
            if not game or not config:
                print("DEBUG: Missing game or config, returning")
                return
            ...
            if game.current_step in [-1, 0, 1] and game.state in [GameState.BETTING, GameState.DEALING]:
                ...
                if hasattr(game, "betting") and hasattr(game.table, "get_position_order"):
                    try:
                        ...
                        print("DEBUG: Reconstructing forced bets from positions")
                        ...
                        if forced_bets:
                            actions = game_action_logger.log_forced_bets(
                                table_id, game.current_step, game.state.value if game.state else "unknown", forced_bets
                            )
                            print(f"DEBUG: Reconstructed {len(forced_bets)} forced bet actions")
                            for action in actions:
                                print(f"DEBUG: Reconstructed action: {action.message}")
                        else:
                            print("DEBUG: No forced bets to reconstruct")
                    ...
                    print("DEBUG: No betting attribute or position order method")
                    self._simple_forced_bet_reconstruction(table_id, game, config)
            else:
    ...
    def _simple_forced_bet_reconstruction(self, table_id: str, game: Game, config: GameConfig):
        ...
        try:
            print("DEBUG: Using simple forced bet reconstruction")
            ...
            if bob_player and erik_player:
                ...
                print("DEBUG: Could not find both players for blind reconstruction")
            ...
            if forced_bets:
                ...
                print("DEBUG: No forced bets created in simple reconstruction")
        ...
        try:
            ...
            for player_id, player_hands in hand_results.hands.items():
                ...
                for hand in player_hands:
                    hand_dict = {
                        ...
                        "community_cards": [str(card) for card in getattr(hand, "community_cards", [])]
                        if getattr(hand, "community_cards", [])
                        else [],
            ...
            for winning_hand in hand_results.winning_hands:
                winning_dict = {
                    ...
                    "community_cards": [str(card) for card in getattr(winning_hand, "community_cards", [])]
                    if getattr(winning_hand, "community_cards", [])
                    else [],
