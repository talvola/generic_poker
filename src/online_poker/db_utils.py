...
from .models import GameHistory, PokerTable, Transaction, User
...
def create_user_with_validation(username: str, email: str, password: str, bankroll: int = 1000) -> User | None:
...
def create_table_with_validation(
    name: str,
    variant: str,
...
def get_user_transaction_history(user_id: str, limit: int = 50) -> list[Transaction]:
    ...
    try:
        transactions = (
            Transaction.query.filter_by(user_id=user_id).order_by(Transaction.created_at.desc()).limit(limit).all()
        )
        return transactions
    except SQLAlchemyError as e:
...
def get_public_tables() -> list[PokerTable]:
    ...
    try:
        tables = PokerTable.query.filter_by(is_private=False).order_by(PokerTable.created_at.desc()).all()
        return tables
    except SQLAlchemyError as e:
...
def get_user_statistics(user_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        game_count = GameHistory.query.join(PokerTable).filter(GameHistory.players.contains(user_id)).count()
...
def execute_raw_query(query: str, params: dict | None = None) -> list[dict]:
