...
def get_variants():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to load poker variants'
        }), 500
...
def get_betting_structures():
    ...
    structures = [
        {
            'value': BettingStructure.LIMIT.value,
            'name': 'Limit',
            'description': 'Fixed betting limits'
        },
        {
...
def get_suggested_stakes(betting_structure: str):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'Invalid betting structure: {betting_structure}'
        }), 400
    except Exception as e:
        current_app.logger.error(f"Failed to get suggested stakes: {e}")
        return jsonify({
            'success': False,
            'error': 'Failed to get suggested stakes'
        }), 500
...
def create_table():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'table': table.to_dict(),
            'message': f'Table "{table.name}" created successfully'
        }), 201
...
def get_table(table_id: str):
    ...
    try:
        ...
        if table.is_private and table.creator_id != current_user.id:
            return jsonify({
                'success': False,
                'error': 'Access denied to private table'
            }), 403
...
def update_table_activity(table_id: str):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Table activity updated'
        })
    except Exception as e:
        current_app.logger.error(f"Failed to update table activity: {e}")
        return jsonify({
            'success': False,
            'error': 'Failed to update table activity'
        }), 500
...
def leave_table(table_id: str):
    ...
    """
    try:
        success, error_msg = TableAccessManager.leave_table(
            user_id=current_user.id,
            table_id=table_id
        )
...
def get_table_players(table_id: str):
    ...
    try:
        ...
        if table.is_private:
            user_access = db.session.query(TableAccess).filter(
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to get table players'
        }), 500
...
def update_table_visibility(table_id: str):
    ...
    try:
        ...
        if not data or 'is_private' not in data:
            return jsonify({
                'success': False,
                'error': 'is_private field required'
            }), 400
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to update table visibility'
        }), 500
...
def regenerate_invite_code(table_id: str):
    ...
    """
    try:
        success, error_msg, new_code = TableManager.regenerate_invite_code(
            table_id=table_id,
            user_id=current_user.id
        )
...
def update_table_settings(table_id: str):
    ...
    try:
        ...
        success, message = TableManager.update_table_settings(
            table_id=table_id,
            user_id=current_user.id,
            settings=data
        )
        ...
        return jsonify({
            'success': True,
            'message': message,
            'table': table.to_dict(include_sensitive=True) if table else None
        })
...
def transfer_host_privileges(table_id: str):
    ...
    try:
        ...
        success, message = TableManager.transfer_host_privileges(
            table_id=table_id,
            current_creator_id=current_user.id,
            new_creator_id=new_host_id
        )
...
def close_table(table_id: str):
    ...
    try:
        ...
        success, message = TableManager.close_table(
            table_id=table_id,
            creator_id=current_user.id,
            reason=reason
        )
