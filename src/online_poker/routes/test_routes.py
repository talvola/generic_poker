...
def cleanup_test_data():
    ...
    try:
        # Find test tables
        test_tables = db.session.query(PokerTable).filter(PokerTable.name.like("Test %")).all()
        ...
        access_deleted = (
            ...
            .filter(TableAccess.table_id.in_(test_table_ids))
            .delete(synchronize_session="fetch")
        )

        # Delete test tables
        tables_deleted = (
            db.session.query(PokerTable).filter(PokerTable.id.in_(test_table_ids)).delete(synchronize_session="fetch")
        )
        ...
        for username, bankroll in seed_bankrolls.items():
            updated = db.session.query(User).filter(User.username == username).update({"bankroll": bankroll})
...
def reset_table_session(table_id: str):
    ...
    try:
        ...
        db.session.query(TableAccess).filter(TableAccess.table_id == table_id).update({"is_ready": False})
        db.session.commit()

        return jsonify({"success": True, "session_cleared": cleared, "message": f"Reset table {table_id}"})
...
def test_status():
    ...
    try:
        ...
        # Count test tables
        test_table_count = db.session.query(PokerTable).filter(PokerTable.name.like("Test %")).count()
