...
def get_tables():
    ...
    try:
        ...
        tables = db.session.query(PokerTable).filter(PokerTable.is_private == False).all()
    ...
    try:
        ...
        if betting_structure == "limit":
            if "small_bet" not in stakes or "big_bet" not in stakes:
                return jsonify({"success": False, "error": "Limit games require small_bet and big_bet"}), 400
        else:
            if "small_blind" not in stakes or "big_blind" not in stakes:
                return jsonify({"success": False, "error": "Blind games require small_blind and big_blind"}), 400
...
def get_table_seats(table_id):
    ...
    try:
        ...
        access_records = (
            db.session.query(TableAccess)
            .filter(TableAccess.table_id == table_id, TableAccess.is_active == True, TableAccess.is_spectator == False)
            .all()
        )
        ...
        for seat_num in range(1, table.max_players + 1):
            if seat_num in occupied_seats:
                ...
                seats.append({"seat_number": seat_num, "is_available": True, "player": None})
...
def join_table(table_id):
    ...
    try:
        ...
        if table.is_private:
            return jsonify({"success": False, "error": "Cannot join private table without invite code"}), 403

        # Check if table is full
        active_players = sum(1 for access in table.access_records if access.is_active and not access.is_spectator)

        if active_players >= table.max_players:
            return jsonify({"success": False, "error": "Table is full"}), 400
        ...
        if buy_in_amount < table.get_minimum_buyin():
            return jsonify({"success": False, "error": f"Buy-in must be at least ${table.get_minimum_buyin()}"}), 400

        if buy_in_amount > table.get_maximum_buyin():
            return jsonify({"success": False, "error": f"Buy-in cannot exceed ${table.get_maximum_buyin()}"}), 400

        # Check if user has sufficient bankroll
        if current_user.bankroll < buy_in_amount:
            return jsonify(
                {"success": False, "error": f"Insufficient bankroll. You have ${current_user.bankroll}"}
            ), 400

        # Join table using table access manager
        success, message, access = TableAccessManager.join_table(
            user_id=current_user.id, table_id=table_id, buy_in_amount=buy_in_amount, seat_number=seat_number
        )

        if success:
            return jsonify({"success": True, "table_id": table_id, "message": "Joined table successfully"})
        else:
...
def join_private_table():
    ...
    try:
        ...
        table = (
            db.session.query(PokerTable)
            .filter(PokerTable.invite_code == invite_code.upper(), PokerTable.is_private == True)
            .first()
        )
        ...
        if success:
            return jsonify({"success": True, "table_id": table.id, "message": "Joined private table successfully"})
        else:
            return jsonify({"success": False, "error": message}), 400
...
def spectate_table(table_id):
    ...
    try:
        ...
        if table.is_private:
            return jsonify({"success": False, "error": "Cannot spectate private table without invite code"}), 403

        # Join as spectator using table access manager
        success, message, access = TableAccessManager.join_table(
            user_id=current_user.id, table_id=table_id, buy_in_amount=0, as_spectator=True
        )

        if success:
            return jsonify({"success": True, "table_id": table_id, "message": "Joined as spectator successfully"})
        else:
            return jsonify({"success": False, "error": message}), 400
...
def register_lobby_socket_events(socketio):
    ...
    def handle_join_table(data):
        ...
        try:
            ...
            if success:
                emit("table_joined", {"table_id": table_id, "message": "Joined table successfully"})

                # Broadcast table update to all lobby users so they see updated player count
                table = db.session.query(PokerTable).filter(PokerTable.id == table_id).first()
                if table:
                    socketio.emit("table_updated", {"table": table.to_dict(), "action": "player_joined"})
            else:
    ...
    def handle_join_private_table(data):
        ...
        try:
            ...
            # Join table using table access manager
            success, message, access = TableAccessManager.join_table(
                user_id=current_user.id, table_id=table.id, buy_in_amount=buy_in_amount, seat_number=seat_number
            )

            if success:
                emit("table_joined", {"table_id": table.id, "message": "Joined private table successfully"})
            else:
                emit("error", {"message": message})
    ...
    def handle_spectate_table(data):
        ...
        try:
            ...
            if success:
                emit("table_joined", {"table_id": table_id, "message": "Joined as spectator successfully"})
            else:
                emit("error", {"message": message})
