    ...
    def create_game(self, game_id, variant, betting_structure, stakes):
        ...
        try:
            ...
            if betting_structure == "limit":
                ...
                game_params.update(
                    {"small_blind": stakes.get("small_blind", 1), "big_blind": stakes.get("big_blind", 2)}
                )
...
def handle_chat_message(data):
    ...
    for _pid, s in player_sids.items():
...
def handle_spectate_game(data):
    ...
    for _pid, s in player_sids.items():
        socketio.emit(
            "spectator_joined", {"name": spectator_name, "message": f"{spectator_name} is now spectating"}, room=s
        )
...
def get_game_state_for_player(player_id):
    ...
    state = {
        ...
        "game_info": {
            ...
            if game.current_step >= 0 and game.current_step < len(game.rules.gameplay)
            else "Not started",
        },
...
class HandHistoryManager:
    ...
    def record_hand(self, game_id, hand_data):
        ...
        self.histories[game_id].append(
            {"hand_number": len(self.histories[game_id]) + 1, "timestamp": datetime.now().isoformat(), **hand_data}
        )
...
def handle_disconnect():
    ...
    if player_id:
        ...
        for _pid, s in player_sids.items():
            socketio.emit(
                "player_disconnected",
                {"player_id": player_id, "player_name": player_name, "timeout_seconds": DISCONNECT_TIMEOUT},
                room=s,
            )
...
def setup_disconnect_timer(player_id):
    ...
    def auto_fold():
        if player_id in disconnected_players and player_id not in player_sids:
            ...
            if game.current_player and game.current_player.id == player_id and game.state == GameState.BETTING:
                try:
                    ...
                    if result.success:
                        ...
                        for pid, s in player_sids.items():
                            ...
                            socketio.emit(
                                "player_auto_folded",
                                {"player_id": player_id, "reason": "disconnection_timeout"},
                                room=s,
                            )
                except Exception as e:
...
def handle_reconnect(data):
    ...
    if player_id:
        ...
        socketio.emit("reconnection_success", {"message": f"Welcome back, {player_name}!"}, room=sid)

        # Notify other players
        for pid, s in player_sids.items():
            if pid != player_id:
                socketio.emit("player_reconnected", {"player_id": player_id, "player_name": player_name}, room=s)
...
def handle_ready(data):
    ...
    for _pid, sid in player_sids.items():
        socketio.emit(
            "ready_status",
            {"ready_players": ready_players, "all_players": all_players, "all_ready": all_ready},
            room=sid,
        )
...
def enhanced_join_handler(data):
    ...
    if game is not None:
        ...
        existing_active = any(
            p.name == player_name and p.id not in disconnected_players for p in game.table.players.values()
        )
        if existing_active:
        ...
        try:
            ...
            while game.state != GameState.COMPLETE:
                ...
                current_state_info = (
                    game.current_step,
                    game.state,
                    game.current_player.id if game.current_player else None,
                )
                ...
                if game.state == GameState.DEALING:
                    ...
                    if step.action_type == GameActionType.CHOOSE:
                        ...
                        print(
                            f"Current player after dealing: {game.current_player.name if game.current_player else 'None'}"
                        )
                    ...
                    try:
                                    ...
                                    player_id: [
                                        {
                                            ...
                                            "used_hole_cards": [str(c) for c in hand.used_hole_cards]
                                            if hand.used_hole_cards
                                            else [],
                                            "rank": hand.rank,
                                        }
                                        for hand in hands
                        ...
                        if attempt < max_attempts - 1:
                            ...
                            simplified_results = {
                                ...
                                "pots": [
                                    {"amount": game.betting.get_total_pot(), "winners": winners, "pot_type": "main"}
                                ],
                                "hands": {},
                                "winning_hands": [],
                            }
        ...
        try:
            ...
            hold_em_config = {
                ...
                "gamePlay": [
                    ...
                    {
                        "name": "Deal Flop",
                        "deal": {"location": "community", "cards": [{"number": 3, "state": "face up"}]},
                    },
                    {"name": "Post-Flop Bet", "bet": {"type": "small"}},
                    {
                        "name": "Deal Turn",
                        "deal": {"location": "community", "cards": [{"number": 1, "state": "face up"}]},
                    },
            ...
            game = Game(
                rules=rules, structure=BettingStructure.NO_LIMIT, small_blind=1, big_blind=2, auto_progress=False
            )
